...
public class BottomNavigationView extends FrameLayout {
    ...
    public BottomNavigationView(Context context, AttributeSet attributeSet, int i) {
        ...
        TintTypedArray obtainStyledAttributes = TintTypedArray.obtainStyledAttributes(context, attributeSet, C0030R.styleable.BottomNavigationView, i, C0030R.style.Widget_Design_BottomNavigationView);
        if (obtainStyledAttributes.hasValue(C0030R.styleable.BottomNavigationView_itemIconTint)) {
            this.mMenuView.setIconTintList(obtainStyledAttributes.getColorStateList(C0030R.styleable.BottomNavigationView_itemIconTint));
        } else {
            this.mMenuView.setIconTintList(createDefaultColorStateList(16842808));
        }
        if (obtainStyledAttributes.hasValue(C0030R.styleable.BottomNavigationView_itemTextColor)) {
            this.mMenuView.setItemTextColor(obtainStyledAttributes.getColorStateList(C0030R.styleable.BottomNavigationView_itemTextColor));
        } else {
            this.mMenuView.setItemTextColor(createDefaultColorStateList(16842808));
        }
        if (obtainStyledAttributes.hasValue(C0030R.styleable.BottomNavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) obtainStyledAttributes.getDimensionPixelSize(C0030R.styleable.BottomNavigationView_elevation, 0));
        }
        this.mMenuView.setItemBackgroundRes(obtainStyledAttributes.getResourceId(C0030R.styleable.BottomNavigationView_itemBackground, 0));
        if (obtainStyledAttributes.hasValue(C0030R.styleable.BottomNavigationView_menu)) {
            inflateMenu(obtainStyledAttributes.getResourceId(C0030R.styleable.BottomNavigationView_menu, 0));
        }
    ...
    private void addCompatibilityTopDivider(Context context) {
        ...
        view.setBackgroundColor(ContextCompat.getColor(context, C0030R.color.design_bottom_navigation_shadow_color));
        view.setLayoutParams(new FrameLayout.LayoutParams(-1, getResources().getDimensionPixelSize(C0030R.dimen.design_bottom_navigation_shadow_height)));
        addView(view);
    }
    ...
    private ColorStateList createDefaultColorStateList(int i) {
        ...
        if (!getContext().getTheme().resolveAttribute(C0359R.attr.colorPrimary, typedValue, true)) {
            return null;
        }
