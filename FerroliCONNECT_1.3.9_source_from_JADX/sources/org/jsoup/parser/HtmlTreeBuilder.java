...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    private static final String[] TagSearchSpecial = {"address", "applet", "area", "article", "aside", "base", "basefont", "bgsound", "blockquote", AgooConstants.MESSAGE_BODY, "br", "button", "caption", "center", "col", "colgroup", "command", "dd", "details", "dir", "div", "dl", "dt", "embed", "fieldset", "figcaption", "figure", "footer", "form", "frame", "frameset", "h1", "h2", "h3", "h4", "h5", "h6", "head", "header", "hgroup", "hr", "html", "iframe", "img", "input", "isindex", "li", "link", "listing", "marquee", "menu", "meta", "nav", "noembed", "noframes", "noscript", "object", "ol", "p", "param", "plaintext", RequestConstant.ENV_PRE, "script", "section", "select", "style", "summary", "table", "tbody", "td", "textarea", "tfoot", "th", "thead", "title", LanguageUtil.LOGOGRAM_TR, "ul", "wbr", "xmp"};
    ...
    public List<Node> parseFragment(String str, Element element, String str2, ParseErrorList parseErrorList, ParseSettings parseSettings) {
        ...
        if (element != null) {
            ...
            if (StringUtil.m3922in(tagName, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else if (StringUtil.m3922in(tagName, "iframe", "noembed", "noframes", "style", "xmp")) {
    ...
    public void popStackToClose(String... strArr) {
        ...
        while (size >= 0) {
            this.stack.remove(size);
            if (!StringUtil.m3922in(((Element) this.stack.get(size)).nodeName(), strArr)) {
                size--;
            } else {
                return;
            }
    ...
    public void clearStackToTableRowContext() {
        clearStackToContext(LanguageUtil.LOGOGRAM_TR);
    }
    ...
    private void clearStackToContext(String... strArr) {
        ...
        while (size >= 0) {
            Element element = (Element) this.stack.get(size);
            if (!StringUtil.m3922in(element.nodeName(), strArr) && !element.nodeName().equals("html")) {
                this.stack.remove(size);
                size--;
            } else {
                return;
    ...
    public void resetInsertionMode() {
        ...
        while (size >= 0) {
            ...
            if ("select".equals(nodeName)) {
                ...
                return;
            } else if (LanguageUtil.LOGOGRAM_TR.equals(nodeName)) {
                ...
                return;
            } else if (AgooConstants.MESSAGE_BODY.equals(nodeName)) {
                transition(HtmlTreeBuilderState.InBody);
                return;
    ...
    private boolean inSpecificScope(String[] strArr, String[] strArr2, String[] strArr3) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            if (StringUtil.m3922in(nodeName, strArr)) {
                return true;
            }
            if (StringUtil.m3922in(nodeName, strArr2)) {
                return false;
            }
            if (strArr3 != null && StringUtil.m3922in(nodeName, strArr3)) {
                return false;
            }
        }
    ...
    public boolean inSelectScope(String str) {
        for (int size = this.stack.size() - 1; size >= 0; size--) {
            ...
            if (!StringUtil.m3922in(nodeName, TagSearchSelectScope)) {
                return false;
            }
        }
        Validate.fail("Should not be reachable");
    ...
    public void generateImpliedEndTags(String str) {
        while (str != null && !currentElement().nodeName().equals(str) && StringUtil.m3922in(currentElement().nodeName(), TagSearchEndTags)) {
            pop();
        }
    }
    ...
    public boolean isSpecial(Element element) {
        return StringUtil.m3922in(element.nodeName(), TagSearchSpecial);
    }
