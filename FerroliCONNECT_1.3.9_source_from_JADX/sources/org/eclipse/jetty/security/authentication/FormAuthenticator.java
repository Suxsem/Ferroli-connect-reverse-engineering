...
public class FormAuthenticator extends LoginAuthenticator {
        ...
        try {
            if (isJSecurityCheck(requestURI)) {
                ...
                if (this._formErrorPage == null) {
                    ...
                    httpServletResponse.setDateHeader(HttpHeaders.EXPIRES, 1);
            ...
            if (authentication != null) {
                if (!(authentication instanceof Authentication.User) || this._loginService == null || this._loginService.validate(((Authentication.User) authentication).getUserIdentity())) {
                    ...
                    if (str2 != null) {
                        MultiMap multiMap = (MultiMap) session.getAttribute(__J_POST);
                        if (multiMap != null) {
            ...
            synchronized (session) {
                if (session.getAttribute(__J_URI) == null || this._alwaysSaveUri) {
                    ...
                    if (MimeTypes.FORM_ENCODED.equalsIgnoreCase(servletRequest.getContentType()) && "POST".equals(httpServletRequest.getMethod())) {
                        ...
                        session.setAttribute(__J_POST, new MultiMap(request2.getParameters()));
                    }
                }
            }
            if (this._dispatch) {
    ...
    protected static class FormResponse extends HttpServletResponseWrapper {
        ...
        private boolean notIgnored(String str) {
            return !"Cache-Control".equalsIgnoreCase(str) && !HttpHeaders.PRAGMA.equalsIgnoreCase(str) && !HttpHeaders.ETAG.equalsIgnoreCase(str) && !HttpHeaders.EXPIRES.equalsIgnoreCase(str) && !HttpHeaders.LAST_MODIFIED.equalsIgnoreCase(str) && !HttpHeaders.AGE.equalsIgnoreCase(str);
        }
    }
