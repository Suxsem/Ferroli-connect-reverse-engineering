...
import okhttp3.internal.p114ws.RealWebSocket;
...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    private boolean isValid(SSLSession sSLSession) {
        return !Constraint.NONE.equals(sSLSession.getProtocol()) && !"SSL_NULL_WITH_NULL_NULL".equals(sSLSession.getCipherSuite());
    }
        ...
        while (true) {
            ...
            if (code != 200) {
                if (code == 407) {
                    ...
                    } else if (HttpHeaderValues.CLOSE.equalsIgnoreCase(build.header(org.eclipse.jetty.http.HttpHeaders.CONNECTION))) {
                        return authenticate;
                    } else {
    ...
    private Request createTunnelRequest() {
        return new Request.Builder().url(this.route.address().url()).header("Host", Util.hostHeader(this.route.address().url(), true)).header(org.eclipse.jetty.http.HttpHeaders.PROXY_CONNECTION, org.eclipse.jetty.http.HttpHeaders.KEEP_ALIVE).header(org.eclipse.jetty.http.HttpHeaders.USER_AGENT, Version.userAgent()).build();
    }
